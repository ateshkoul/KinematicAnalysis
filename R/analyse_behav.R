...
#     DataFull <- rbind(DataFull,Data)
  ...
  DataFull$Sub <- as.numeric(levels(DataFull$Sub)[DataFull$Sub])
  DataFull$Trial <- as.numeric(levels(DataFull$Trial)[DataFull$Trial])
  ...
  ClusterResults <- ddply(DataFull,.(Subject,Pouring,Drinking),summarise,clustAcc = mean(ACC,na.rm=T),Condizione=unique(Condizione),Age = unique(Age))
  
  

  
  addWorksheet(SubResults,"Combined_Results")
  writeData(SubResults,"Combined_Results",DataFull)
